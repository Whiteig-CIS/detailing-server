...
app.get("/api/packages/:id", async (req, res) => {
  const pkg = PackageDB.findOne({_id: id});
    res.send(pkg);
});
...
app.post("/api/packages", upload.array("images", 20), async (req, res) => {
  ...
  const pkg = new PackageDB({ //construct the new package with all the non-file elements
  ...
  if (req.files) { // seperate before and after photos
    ...
    if (req.files.length !== 0) {
      ...
      pkg.preview_image = req.files[1].filename; // setting the preview image to the first before picture for now.
...
app.put("/api/packages/:id", upload.array("images", 20), async (req, res) => {
  ...
  let feildsToUpdate = {
    ...
    starting_price : req.body.starting_price,
    summary : req.body.summary,
    interior_services : req.body.interior_services.split(","),
    exterior_services : req.body.exterior_services.split(",")
  }

  const wentThrough = await PackageDB.updateOne(
    { _id: req.params.id }, // find a package that matches this
    feildsToUpdate // update that house to this
  ) ; 
...
app.post("/api/packages", upload.array("images", 20), async(req, res) => {
  ...
  const pkg = new Package({ //construct the new package with all the non-file elements
    vehicle_type: req.body.vehicle_type,
    teir: req.body.teir,
    starting_price: req.body.starting_price,
    summary: req.body.summary,
...
app.put("/api/packages/:id", upload.array("images", 20), async(req, res) => {
  ...
  // TODO do the images section

  const success = await Package.updateOne({_id:req.params.id}, fieldsToUpdate);
